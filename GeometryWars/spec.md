...
- 颜色：纯白色 `COLOR_ARGB(255, 255, 255, 255)`（经典街机风格，不和任何敌人颜色撞）
- 光晕阴影：偏移 (x+1, y+1) 先绘制白色半透明光晕（alpha 随主文字 alpha * 100），黑底上形成醒目背光效果
- 以 60px/s 速度向上飘动
- 1 秒生命周期，alpha 渐隐
- 最多同时存在 30 个
...
- 最后 0.5s：alpha 渐隐
- 屏幕中央显示，每局每种成就只触发一次
- **阴影**：在正文偏移 (x+1, y+1) 处先绘制白色半透明光晕（alpha 100），黑底上形成醒目背光效果
